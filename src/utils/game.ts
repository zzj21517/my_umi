type TImg = ['backgroundImg', 'backgroundReady'] | ['monsterImg', 'monsterReady'] | ['humterImg', 'humterReady']
interface Role {
  x: number,
  y: number,
}
...
class Game {
  ...
  initImg([imgName, isImgReadyName]: TImg) {
    ...
    if (this[imgName] as HTMLImageElement) {
      ...
      (this[imgName] as HTMLImageElement).src = require(`../asserts/images/${imgName}.png`)
    }
  }
  ...
  draw() {
    if (this.context instanceof CanvasRenderingContext2D) {
      if (this.backgroundReady && this.backgroundImg instanceof HTMLImageElement) {
  ...
  // 是否抓住怪兽
  isCaughtMonster() {
    if (this.humter.x >= this.monster.x - 32 && this.humter.x <= this.monster.x + 32 && this.humter.y >= this.monster.y - 32 && this.humter.y <= this.monster.y + 32) {
      this.caughtMonster++
      this.reset()
    }
  }
